...
export class FoodLogPage {
  ...
  */
  ...
  constructor(
    ...
    this.selectedDate = navParams.get('selectedDate') || moment().format('DD/MM/YYYY');
  ...
  getMeals() {
    this.dbServ.meals.then((meals: MealOut[]) => {
      ...
      this.break = this.formatRows(byDate.filter(f => f.meal === 'Breakfast'));
      this.lunch = this.formatRows(byDate.filter(f => f.meal === 'Lunch'));
      this.dinner = this.formatRows(byDate.filter(f => f.meal === 'Dinner'));
      this.snack = this.formatRows(byDate.filter(f => f.meal === 'Snack'));
