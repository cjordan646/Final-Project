...
export class DetailsPage {
  ...
  constructor(public navCtrl: NavController, public formbuilder: FormBuilder, private dbServ: DatabaseService) {
    ...
    this.dbServ.details.then((data: DetailOut[]) => {
      if (data.length > 0) {
        ...
        this.myForm.get('gender').setValue(row.gender);
        this.myForm.get('age').setValue(row.age);
        this.myForm.get('weight').setValue(row.weight);
      }
    })
  }
  ...
  SubmitDetails() {
    ...
    let promise;
    if (this.detailsId !== '') {
      this.dbServ.update('details', this.detailsId, model);
    } else {
      this.dbServ.insert('details', model)
    }
